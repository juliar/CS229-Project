...
'removing zeros'
DATA1NOZEROS = repmat(0, [size(DATA1, 1) size(DATA1, 2)]);
zeroRow = zeros(1, size(DATA1, 2));
nonZeroRowNum = 0;
for rowNum = 1:size(DATA1, 1)
  if DATA1(rowNum, :) ~= zeroRow
    nonZeroRowNum = nonZeroRowNum + 1;
    DATA1NOZEROS(nonZeroRowNum, :) = DATA1(rowNum, :);
  end
end 

'averaging'
AVGDATA1 = repmat(0, (int32(floor(nonZeroRowNum/160))), (size(DATA1NOZEROS, 2)));
int32(floor(nonZeroRowNum/160))
nonZeroRowNum - mod(nonZeroRowNum,160)
for rowNum = 1:(nonZeroRowNum - mod(nonZeroRowNum,160))
  avgRowNum = int32(floor((rowNum-1)/160))+1;

  for col=1:size(DATA1,2) 
     AVGDATA1(avgRowNum,col) = AVGDATA1(avgRowNum,col) + DATA1NOZEROS(rowNum,col);
  end
end
...
'removing zeros'
DATA2NOZEROS = repmat(0, [size(DATA2, 1) size(DATA2, 2)]);
zeroRow = zeros(1, size(DATA2, 2));
nonZeroRowNum = 0;
for rowNum = 1:size(DATA2, 1)
  if DATA2(rowNum, :) ~= zeroRow
    nonZeroRowNum = nonZeroRowNum + 1;
    DATA2NOZEROS(nonZeroRowNum, :) = DATA2(rowNum, :);
  end
end 

'averaging'
AVGDATA2 = repmat(0, (int32(floor(nonZeroRowNum/160))), (size(DATA2NOZEROS, 2)));
for rowNum = 1:(nonZeroRowNum - mod(nonZeroRowNum,160))
  avgRowNum = int32(floor((rowNum-1)/160))+1;

  for col=1:size(DATA2,2) 
     AVGDATA2(avgRowNum,col) = AVGDATA2(avgRowNum,col) + DATA2NOZEROS(rowNum,col);
  end
end
...
M = [M; ons AVGDATA2];
'writing csv'
csvwrite('features_nozeros.csv', M);
